...
import { getSearchEntities, getEntities } from './travel.reducer';
    ...
    return (
      <div>
        <h2 id="travel-heading">
          <Translate contentKey="letsgoApp.travel.home.title">Travels</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="letsgoApp.travel.home.createLabel">Create new Travel</Translate>
          </Link>
        </h2>
        <Row>
          <Col sm="12">
              ...
              <AvGroup>
                <InputGroup>
                  <AvInput
                    ...
                    placeholder={translate('letsgoApp.travel.home.search')}
                  />
            ...
            <tbody>
              {travelList.map((travel, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{travel.number}</td>
                  <td>
                    <TextFormat type="date" value={travel.date} format={APP_DATE_FORMAT} />
                  </td>
                  <td>{travel.nbPlace}</td>
                  <td>{travel.leftPlace}</td>
                  <td>{travel.planning ? <Link to={`planning/${travel.planning.id}`}>{travel.planning.id}</Link> : ''}</td>
